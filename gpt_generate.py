...
def main():
    ...
    for user_agent_prompt_base in tqdm.tqdm(user_agent_prompts_base):
        ...
        outputs.append(completion.choices[0].message.content.strip().split('\n'))

    my_utils.pickle_write(outputs, 'llm_outputs/gpt4o_20241120_AtLeastOneAction_CoT.pickle', overwrite=True)
    print(f'実行時間: {(running_time) / 60:.3f}min')
